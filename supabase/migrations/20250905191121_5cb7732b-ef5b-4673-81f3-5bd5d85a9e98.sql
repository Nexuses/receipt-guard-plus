-- Add approval fields without foreign key constraint (since auth.users is not directly accessible)
...
-- Now safely drop and recreate the status constraint
ALTER TABLE public.expenses 
DROP CONSTRAINT IF EXISTS expenses_status_check;

-- Add comprehensive status constraint with approval workflow
ALTER TABLE public.expenses 
